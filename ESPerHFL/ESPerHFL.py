...
def fast_all_clients_test(v_test_loader, global_nn,local, device, q,mix):
    n = 0.0
    m = 0.0

    with torch.no_grad():
        l = 0
        for dataset in v_test_loader:
            for data in dataset:
                inputs, labels = data
                inputs = inputs.to(device)
                labels = labels.to(device)
                mixt = global_nn
                global_dict = global_nn.state_dict()
                mixt_dict = mixt.state_dict()
                for par1, par2, par3 in zip(mixt_dict, local, global_dict):
                     mixt_dict[par1] = mix * local[par2] + (1 - mix) * global_dict[par3]
                mixt.load_state_dict(mixt_dict)

                outputs = mixt(inputs)
                _, predicts = torch.max(outputs, 1)
                m += labels.size(0)
                n += (predicts == labels).sum().item()
                l += 1
...
def initialize_global_nn(args):
    if args.dataset == 'mnist':
        if args.model == 'lenet':
            global_nn = mnist_lenet1(input_channels=1, output_channels=10)
...
def alpha_update(model_local, model_personal, alpha):
    grad_alpha = 0
...
def HierFAVG(args):
    ...
    if args.iid == -2:
        ...
        for i in range(args.num_edges):
            ...
            selected_cids = [5*i,5*i+1,5*i+2,5*i+3,5*i+4]
            cids = list (set(cids) - set(selected_cids))
            edges.append(Edge(id = i,
                              cids = selected_cids,
                              shared_layers = copy.deepcopy(clients[0].shared_layers)))
    ...
    cloud = Cloud(shared_layers=copy.deepcopy(clients[0].shared_layers))
    ...
    for num_comm in tqdm(range(args.num_communication)):
            ...
            for i,edge in enumerate(edges):
                ...
                for selected_cid in selected_cids:
                    ...
                    client_loss += clients[selected_cid].local_update(num_iter=args.num_local_update,
                                                                      device = device)
                    clients[selected_cid].m =alpha_update(clients[selected_cid].shared_layers, clients[selected_cid].local, clients[selected_cid].m)
        ...
        o=0
        for edge in edges:
            cloud.send_to_edge(edge,o)
            o=o+1


        correct_all = 0.0
        total_all = 0.0
        for q in range(args.num_edges):
            global_nn.load_state_dict(state_dict = copy.deepcopy(edges[q].shared_state_dict))
            global_nn.train(False)



            correct_all_v, total_all_v = fast_all_clients_test(v_test_loader[5*q:5*(q+1)], global_nn,edges[q].local_model, device,q,edges[q].mix)
            correct_all += correct_all_v
            total_all += total_all_v

        avg_acc_v = correct_all / total_all
